...
class Model():
    def __init__(self, args):
        ...
        #layer: name conv2d outputshape [None, 16, 127, 64]
        #layer: name Mean outputshape [None, 127, 64]

        #majority = KK.layers.average([baseadj[:,xx] for xx in range(16)])
        # this can be thought of a (16,1) kernel
        #majority = KK.backend.mean(baseadj, axis=[1])
        majorityIn = KK.layers.Conv2D(127, kernel_size= (4, 4), activation='relu')(baseadj)
        ...
        layerB = KK.layers.Dense(64, activation='relu')(lengthszero)
        
        predictionsHPLEN = KK.layers.Dense(33, activation='softmax')(layerB)
